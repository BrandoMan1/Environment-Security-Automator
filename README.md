### Brandon R - ⚙️ Automated Environment Setup: setup_dev_env.sh 
...
The `setup_dev_env.sh` script automates the full setup and configuration of a standardized development environment on a newly provisioned server (Ubuntu/CentOS). It solves the critical problem of manual setup inconsistency by performing the following core actions:
  ...
  - Secure Configuration: It prompts the user for project details and securely generates a personalized configuration file (~/.<ProjectName>.config.yaml).
...
**🚀 Execution Guide**  
How to Run
The script requires execution permission and should be run interactively by a user with sudo privileges (e.g., devops-brandon).
``` bash
...
The `setup_dev_env.sh` script relies on the following packages and privileges to execute successfully:
...
**System Utilities (Assumed Pre-installed)**
These utilities are used for core script logic and logging:
